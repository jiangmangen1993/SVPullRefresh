...
{
    BOOL _bOriginalTopInsetRested;
    BOOL _bOriginalBottomInsetReseted;
}
...
@property (nonatomic, assign, readwrite) CGFloat progress;   //[ricky add]
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        //---- ricky add TBU refresh view as custom view ---
        TBURefreshView *v = [[TBURefreshView alloc] initWithFrame:CGRectMake(0, 0, 130, 20)];
        [view setCustomView:v forState:SVPullToRefreshStateAll];
    }
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2), roundf((self.bounds.size.height-viewBounds.size.height)/2) + 6);
        [customView setFrame:CGRectMake(origin.x, origin.y, viewBounds.size.width, viewBounds.size.height)];
        
        if ([customView respondsToSelector:@selector(updateProgress:forState:)]){
            [customView updateProgress:self.progress forState:self.state];
        }
    }
    else {
        switch (self.state) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    {
        UIEdgeInsets ine = [[change valueForKey:NSKeyValueChangeNewKey] UIEdgeInsetsValue];
        if(!_bOriginalTopInsetRested && ine.top > 0 && self.state != SVPullToRefreshStateLoading)
        {
            self.originalTopInset = ine.top;
            _bOriginalTopInsetRested = YES;
        }
        if(!_bOriginalBottomInsetReseted && ine.bottom > 0 && self.state != SVPullToRefreshStateLoading){
            self.originalBottomInset = ine.bottom;
            _bOriginalBottomInsetReseted = YES;
        }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        //[ricky add]
        if(self.scrollView.isDragging && self.state == SVPullToRefreshStateStopped){
            if(self.position == SVPullToRefreshPositionTop){
                self.progress = MAX(0, MIN(1.f, (-contentOffset.y - self.originalTopInset)/SVPullToRefreshViewHeight));
            }
            else{
                self.progress = MAX(0, MIN(1.f, 1 - (scrollOffsetThreshold - contentOffset.y)/SVPullToRefreshViewHeight));
            }
...
- (void)refreshCustomView{
    ...
    if (hasCustomView && [customView respondsToSelector:@selector(updateProgress:forState:)]){
        [customView updateProgress:self.progress forState:self.state];
    }
}
...
- (void)startAnimating{
    switch (self.position) {
        case SVPullToRefreshPositionTop:
            if(fequalzero(self.scrollView.contentOffset.y) || self.originalTopInset > 0) {
